...
ANIMATION_TIME_WINDOW_US = 100.0
ANIMATION_PROFILE_RUN_TIME = 60.0
...
class VisualFactory:
    ...
    def ladder(self, sections: int = 20, width: float = 10.6) -> VGroup:
        group = VGroup()
        x_values = np.linspace(-width / 2, width / 2, sections + 1)
        y_top = 0.35
        y_ground = -0.75

        ground = Line(
            np.array([x_values[0], y_ground, 0.0]),
            np.array([x_values[-1], y_ground, 0.0]),
            color=PANEL_STROKE,
            stroke_width=2,
        )
        group.add(ground)
        ...
        for index in range(sections):
            start = np.array([x_values[index], y_top, 0.0])
            end = np.array([x_values[index + 1], y_top, 0.0])
            section_color = BLUE if index % 2 == 0 else CYAN
            group.add(Line(start, end, color=section_color, stroke_width=3))

        for index, x_value in enumerate(x_values):
            dot_radius = 0.035 if index not in (0, sections) else 0.055
            group.add(Dot(np.array([x_value, y_top, 0.0]), radius=dot_radius, color=TEXT))
            if index not in (0, sections):
                plate_y = -0.12
                wire = Line(
                    np.array([x_value, y_top, 0.0]),
                    np.array([x_value, plate_y + 0.12, 0.0]),
                    color=MUTED,
                    stroke_width=1.4,
                )
                plate_a = Line(
                    np.array([x_value - 0.055, plate_y + 0.04, 0.0]),
                    np.array([x_value + 0.055, plate_y + 0.04, 0.0]),
                    color=YELLOW,
                    stroke_width=1.8,
                )
...
class SurgePresentation(Scene):
    ...
    def construct(self) -> None:
        ...
        self.model_scene()
        self.source_scene()
        self.travelling_wave_scene()
        self.reflection_scene()
    ...
    def model_scene(self) -> None:
        cfg = self.data.config
        sections = int(cfg["n_sections"])
        heading = self.factory.heading("Distributed Electrical Model", f"{sections} cascaded sections, each carrying R, L, and C")
        ladder = self.factory.ladder(sections=sections).scale(0.95).shift(UP * 0.55)
        left_label = Text("input", font_size=14, color=ORANGE).next_to(ladder, LEFT, buff=0.12)
        right_label = Text("open end", font_size=14, color=RED).next_to(ladder, RIGHT, buff=0.12)

        per_section = VGroup(
            Text(f"L_total = {float(cfg['L_total']) * 1e3:.1f} mH", font_size=16, color=CYAN),
            Text(f"R_total = {float(cfg['R_total']):.1f} ohm", font_size=16, color=ORANGE),
            Text(f"C_total = {float(cfg['C_total']) * 1e9:.1f} nF", font_size=16, color=YELLOW),
            Text("State-space ODE solved with scipy", font_size=16, color=MUTED),
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.14)
        per_section.to_edge(DOWN, buff=0.45).shift(LEFT * 3.3)

        pi_model = self.factory.mini_pi_section().scale(0.85).shift(RIGHT * 2.0 + DOWN * 1.55)
        t_model = self.factory.mini_t_section().scale(0.85).shift(RIGHT * 5.0 + DOWN * 1.55)

        self.play(FadeIn(heading), run_time=0.7)
        self.play(Create(ladder), FadeIn(left_label), FadeIn(right_label), run_time=1.8)
        self.play(FadeIn(per_section, shift=RIGHT * 0.2), run_time=0.8)
        self.play(FadeIn(pi_model, shift=UP * 0.15), FadeIn(t_model, shift=UP * 0.15), run_time=1.1)
    ...
    def source_scene(self) -> None:
        heading = self.factory.heading(
            "Impulse Source",
            "Double-exponential 1.2/50 us voltage waveform - first 100 us",
        )
        pi = self.data.pi_nodes
        cfg = self.data.config
        time_us = pi.time_s * 1e6
        source_v = pi.voltages[0]
        display_end_us = self.time_window_end_us(pi, STATIC_TIME_WINDOW_US)
        sample = self.sample_time_indices(time_us, display_end_us, 520)
        x_step_us = 20.0 if display_end_us > 40.0 else 5.0
        axes = Axes(
            x_range=[0, display_end_us, x_step_us],
            y_range=[0, 1100, 250],
            x_length=11.2,
            y_length=4.2,
            tips=False,
        ...
        labels = axes.get_axis_labels(
            Text("time (us)", font_size=16, color=MUTED),
            Text("voltage (V)", font_size=16, color=MUTED),
        )
        graph = self.factory.line_graph(axes, time_us[sample], source_v[sample], ORANGE, width=3.4)
        peak_index = int(np.argmax(source_v))
        peak_point = axes.c2p(float(time_us[peak_index]), float(source_v[peak_index]))
        peak_dot = Dot(peak_point, color=YELLOW, radius=0.07)
        peak_label = Text("normalized 1000 V peak", font_size=16, color=YELLOW)
        peak_label.next_to(peak_dot, UP + RIGHT, buff=0.15)
        front_x = float(cfg["t_front"]) * 1e6
        tail_x = float(cfg["t_tail"]) * 1e6
        front_line = DashedLine(axes.c2p(front_x, 0), axes.c2p(front_x, 880), color=CYAN, stroke_width=2)
        front_label = Text("front 1.2 us", font_size=14, color=CYAN).next_to(front_line, RIGHT, buff=0.08)
        if tail_x <= display_end_us:
            tail_line = DashedLine(axes.c2p(tail_x, 0), axes.c2p(tail_x, 520), color=BLUE, stroke_width=2)
            tail_label = Text("tail 50 us", font_size=14, color=BLUE).next_to(tail_line, LEFT, buff=0.08)
            tail_note = VGroup(tail_line, tail_label)
        else:
            tail_note = Text("tail parameter: 50 us", font_size=15, color=BLUE)
            tail_note.move_to(axes.c2p(display_end_us * 0.76, 1010))
    ...
    def travelling_wave_scene(self) -> None:
        heading = self.factory.heading("Travelling Wave", "The real CSV data drives the voltage profile animation")
        data = self.data.pi_nodes
        time_us = data.time_s * 1e6
        animation_end_us = self.time_window_end_us(data, ANIMATION_TIME_WINDOW_US)
        axes = Axes(
            x_range=[0, 100, 20],
            y_range=[-250, 2250, 500],
            x_length=10.2,
            y_length=4.3,
            tips=False,
            axis_config={"color": MUTED, "stroke_width": 2},
        ...
        labels = axes.get_axis_labels(
            Text("coil position (%)", font_size=15, color=MUTED),
            Text("voltage (V)", font_size=15, color=MUTED),
        )
        ...
        self.play(FadeIn(heading), Create(axes), FadeIn(labels), run_time=1.0)
        self.play(FadeIn(profile), FadeIn(time_label), GrowArrow(input_arrow), FadeIn(input_text), run_time=0.8)
        self.play(
            tracker.animate.set_value(animation_end_us),
            run_time=ANIMATION_PROFILE_RUN_TIME,
            rate_func=linear,
        )
        self.play(Indicate(profile, color=YELLOW), run_time=0.8)
        self.wait(0.3)
        self.clear()
    ...
    def grounded_return_scene(self, clear_after: bool = True) -> None:
        heading = self.factory.heading(
            ...
            "The far end returns to reference; 19 segment dV shares sum to 100%",
        )
        ...
        def make_segment_percentage_row() -> VGroup:
            y_values = current_voltage_profile()
            percentages = self.segment_voltage_percentages(
                data.positions,
                y_values,
            ...
            for index, percentage in enumerate(percentages):
                ...
                label = Text(f"{percentage:.0f}%", font_size=8, color=TEXT)
                label.move_to(cell_center)
                cells.add(VGroup(cell, label))

            title = Text("19 segment dV shares", font_size=13, color=MUTED)
            title.next_to(cells, UP, buff=0.08)
            total_percentage = float(np.sum(percentages))
            total_color = GREEN if total_percentage > 99.0 else MUTED
            total_label = Text(f"sum {total_percentage:.0f}%", font_size=12, color=total_color)
            total_label.next_to(cells, RIGHT, buff=0.14)
            return VGroup(title, cells, total_label)

        profile = always_redraw(make_profile)
        percentage_row = always_redraw(make_segment_percentage_row)
        time_label = always_redraw(
            lambda: Text(
                f"t = {tracker.get_value():.2f} us",
                font_size=22,
        ...
        source_label = Text("source input", font_size=15, color=ORANGE)
        source_label.move_to(axes.c2p(12.0, y_limit * 0.74))
